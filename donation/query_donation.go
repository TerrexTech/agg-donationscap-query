...
func QueryDonation(collection *mongo.Collection, event *model.Event, findopts ...findopt.Find) *model.KafkaResponse {
	...
	if err != nil {
		err = errors.Wrap(err, "Query: Error marshalling Donation Delete-result")
		log.Println(err)
		return &model.KafkaResponse{
			AggregateID:   event.AggregateID,
			CorrelationID: event.CorrelationID,
