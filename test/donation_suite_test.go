...
import (
	...
	"github.com/TerrexTech/agg-donationscap-query/donation"
	...
	Describe("Donation Operations", func() {
		It("should query record", func(done Done) {
			...
			queryArgs := map[string]interface{}{
				"donationID": "a08c2f85-150d-41a0-a2b9-567a1d268cee",
			}
			...
			msgCallback := func(msg *sarama.ConsumerMessage) bool {
				...
				if kr.UUID == mockEvent.UUID {
					...
					for _, r := range result {
						if r.ItemID == mockDonate.ItemID {
							mockDonate.ID = r.ID
							Expect(r).To(Equal(*mockDonate))
							return true
						}
					}
				}
